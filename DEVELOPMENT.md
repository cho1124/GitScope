...
- **Phase 8-D: interactive rebase MVP ✅ (2026-04-29, reorder + drop)** — 컨텍스트 메뉴 "Interactive rebase from here" → `InteractiveRebaseModal` (↑↓ 재정렬 + 체크박스 드롭). `git rebase -i` 의 todo 파일 직접 다루지 않고 **cherry-pick 체인**으로 안전 구현: clean tree 검증 → reset --hard from → 각 op cherry-pick → 충돌 시 abort + 원본 HEAD로 reset --hard 자동 롤백. squash/fixup/reword/edit는 차후 작업
